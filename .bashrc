...
alias wua='for ifx in $(basename -s .conf $(compgen -G "/etc/wireguard/*.conf")) ; do echo $ifx ; wu $ifx ; done'
alias wda='for ifx in $(basename -s .conf $(compgen -G "/etc/wireguard/*.conf")) ; do echo $ifx ; wd $ifx ; done'
...
__git_local_10() {
    ...
    git lol --color=always -10 $(git branch | grep -v -e 'HEAD detached' -e '(no branch' | cut -c3- | paste -sd' ')
}
